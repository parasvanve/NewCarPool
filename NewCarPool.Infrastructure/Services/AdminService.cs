    ...
    public async Task<IReadOnlyList<RideOfferDto>> GetRidesAsync(CancellationToken cancellationToken) =>
        await _rides.Query().Include(x => x.Driver).Include(x => x.IntermediateStops).OrderByDescending(x => x.CreatedAtUtc).Select(x => new RideOfferDto(
            ...
            x.IntermediateStops.OrderBy(s => s.StopOrder)
                ...
                .ToList(),
            x.DepartureTimeUtc, x.AvailableSeats, x.Bookings.Count(b => b.Status == BookingStatus.Confirmed), x.PricePerSeat, x.Notes, x.VehicleName, x.VehicleNumber, x.RoutePolyline, x.DistanceKm, x.EtaMinutes, x.Status,
            x.StartedAtUtc, x.CompletedAtUtc, x.CancelledAtUtc, x.CancellationReason)).ToListAsync(cancellationToken);
