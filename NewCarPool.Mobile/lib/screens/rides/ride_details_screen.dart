  ...
  Future<void> _startRide() async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
            'Passengers will be notified and live tracking will begin. Continue?'),
        actions: [
          TextButton(
    ...
    try {
      final updated = await context.read<RideService>().startRide(ride.id);
      if (!mounted) return;
      setState(() => _ride = updated);
      await _syncTracking(updated);
      if (mounted) {
  ...
  Future<void> _cancelRide() async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            const Text(
                'All confirmed passengers will be notified. Are you sure?'),
    ...
    try {
      final updated = await context
          .read<RideService>()
          .cancelRide(ride.id, reason: reasonController.text);
      if (!mounted) return;
      setState(() => _ride = updated);
      await context.read<TrackingService>().stopAll();
      await _syncTracking(updated);
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          if (isDriver && ride.status != 4 && ride.status != 5)
            IconButton(
          ...
          // Passenger calling the driver from the top app bar.
          IconButton(
            icon: const Icon(Icons.call_outlined),
            onPressed: (ride.driverPhoneNumber == null ||
                    ride.driverPhoneNumber!.isEmpty)
                ? null
                : () =>
                    launchUrl(Uri(scheme: 'tel', path: ride.driverPhoneNumber)),
          ),
        ],
      ),
      body: SafeArea(
                        ...
                        children: [
                          ...
                          Row(
                            children: [
                              ...
                              // FIX: This single Builder button already
                              // handles all three states (Start / Complete /
                              // View on Map) via canStart / canComplete.
                              // The old duplicate "Start Ride" /
                              // "Complete Ride" blocks that used to follow
                              // this Expanded have been removed — they were
                              // causing two buttons to render at once.
                              Expanded(
                                child: Builder(builder: (context) {
                                  final canStart = isDriver &&
                                      !isStarted &&
                                      (ride.status == 1 || ride.status == 2);
                                  final canComplete = isDriver && isStarted;
      ...
      // FIX: mobile bottom bar now mirrors the same canStart / canComplete
      // logic as desktop, so it correctly shows "Start Ride" before the
      // ride begins instead of jumping straight to "View on Map".
        ...
        _Card(
          child: Column(
            ...
            children: [
              ...
              _KVRow('Seat Booked', '${myBooking?.seatsBooked ?? 1} Seat'),
          ...
          child: Row(
            children: [
              ...
              IconButton(
                  onPressed: onChat,
                  icon: const Icon(Icons.chat_bubble_outline)),
              // Passenger calling the driver from this card.
              IconButton(
                onPressed: (ride.driverPhoneNumber == null ||
                        ride.driverPhoneNumber!.isEmpty)
                    ? null
                    : () => launchUrl(
                        Uri(scheme: 'tel', path: ride.driverPhoneNumber)),
                icon: const Icon(Icons.call_outlined),
              ),
            ],
          ),
        ),
        // Driver calling any of the passengers on this ride.
        // Only shown to the driver — a passenger never sees this card.
        if (isDriver) _PassengersCard(rideId: ride.id),
        _Card(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
...
/// Shows every passenger booked on this ride to the driver, each with
/// their own call button — so a driver can reach any passenger before,
/// during, or after the ride, no matter how many seats are booked.
class _PassengersCard extends StatelessWidget {
  const _PassengersCard({required this.rideId});
  final String rideId;
      ...
      child: Column(
        ...
        children: [
          ...
          FutureBuilder<List<RideBooking>>(
            future: context.read<RideProvider>().participants(rideId),
                            ...
                            children: [
                              ...
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    Text(
                                      p.passengerName.isEmpty
                                          ? 'Passenger'
                                          : p.passengerName,
                                      style: const TextStyle(
                                          fontWeight: FontWeight.w600),
                                    ),
                                    Text('${p.seatsBooked} seat(s)',
                                        style: TextStyle(
                                            fontSize: 12,
                              ...
                              IconButton(
                                ...
                                onPressed: (p.passengerPhoneNumber == null ||
                                        p.passengerPhoneNumber!.isEmpty)
                                    ? null
                                    : () => launchUrl(Uri(
                                        scheme: 'tel',
                                        path: p.passengerPhoneNumber)),
                              ),
                            ],
                          ),
                        ))
...
//           ),
//           // Passenger calling the driver from the top app bar.
//           // Hidden for the driver themself — a driver calling their own
...
//   @override
//   Widget build(BuildContext context) {
//     // Driver view: fetch the passenger list ONCE here, and reuse the exact
//     // same list for both the "Seat(s) Booked" total and the Passengers
//     // section below. This is what fixes the mismatch — both numbers now
//     // always come from the same data instead of two separate API calls.
//     if (isDriver) {
//       return FutureBuilder<List<RideBooking>>(
//         future: context.read<RideProvider>().participants(ride.id),
//         builder: (context, snapshot) {
//           final passengers = snapshot.data ?? const <RideBooking>[];
//           final isLoadingPassengers =
//               snapshot.connectionState == ConnectionState.waiting;
//           final totalSeats =
//               passengers.fold<int>(0, (sum, p) => sum + p.seatsBooked);
//           final seatsLabel = isLoadingPassengers
//               ? '...'
//               : totalSeats == 0
//                   ? 'No seats booked yet'
...
//           return _buildPanel(
//             context,
//             seatsBookedLabel: seatsLabel,
//             passengersCard: _PassengersCard(
...
//     // Passenger view: unchanged — show the seats from their own booking.
//     final seats = myBooking?.seatsBooked ?? 0;
//     final seatsLabel =
//         seats == 0 ? 'Not booked' : '$seats ${seats == 1 ? 'Seat' : 'Seats'}';
...
//     required String seatsBookedLabel,
//     required Widget? passengersCard,
//   }) {
//     final started = ride.status == 3;
//     final timeline = <(String, String, Color)>[
//       (
...
//                   DepartureTimeUtils.formatFriendly(ride.departureTimeUtc,
//                       context: 'Ride Details Summary')),
//               _KVRow('Seat(s) Booked', seatsBookedLabel),
//               _KVRow('Booking Status', _statusLabel(ride.status)),
//             ],
//           ),
//         ),
...
//             ),
//           ),
//         // Driver-info card. Only shown to a PASSENGER (so they can see and
//         // call/chat the driver). The driver never sees this — a driver
//         // calling or chatting their own number/self would make no sense.
//         if (!isDriver)
//           _Card(
//             child: Row(
//               children: [
...
//         // Driver calling any of the passengers on this ride.
//         // Only shown to the driver — a passenger never sees this card.
//         if (passengersCard != null) passengersCard,
//         _Card(
//           child: Column(
//             crossAxisAlignment: CrossAxisAlignment.start,
//             children: [
...
// /// Shows every passenger booked on this ride to the driver, each with
// /// their own call button. Receives an already-fetched list (from
// /// _RightPanel) instead of fetching its own, so the numbers shown here
// /// always match the "Seat(s) Booked" total above — no mismatch.
// class _PassengersCard extends StatelessWidget {
//   const _PassengersCard({required this.passengers, required this.isLoading});

//   final List<RideBooking> passengers;
...
//             const Text('No passengers booked yet.')
//           else
//             Column(
//               children: passengers
//                   .map((p) => Padding(
...
//                                   ),
//                                   Text(
//                                     '${p.seatsBooked} ${p.seatsBooked == 1 ? 'seat' : 'seats'}',
//                                     style: TextStyle(
//                                         fontSize: 12,
//                                         color: Colors.grey.shade600),
//                                   ),
